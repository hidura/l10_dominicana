...
class AccountMove(models.Model):
    ...
    def _compute_l10n_do_enable_first_sequence(self):
        ...
        if self.l10n_do_enable_first_sequence:
            cf_sequence = self.env['sequence.cf'].\
                search([('l10n_latam_document_type_id', '=',self.l10n_latam_document_type_id.id)], limit=1)

            sequence = str(cf_sequence.current_sequence).zfill(8)

            self.update({'l10n_latam_document_number':self.l10n_latam_document_type_id.doc_code_prefix+sequence})
    ...
    def _move_autocomplete_invoice_lines_create(self, vals_list):
        if 'l10n_latam_sequence_id' in vals_list:
            del vals_list['l10n_latam_sequence_id']
    ...
    def _get_last_sequence(self, relaxed=False):
        ...
        ncf_last_sequence=self.env.cr.fetchone()

        return (ncf_last_sequence or [None])[0]
    ...
    def new(self, values={}, origin=None, ref=None):
        ...
        if 'l10n_latam_sequence_id' in values:
            del values['l10n_latam_sequence_id']
        if 'ncf_expiration_date' in values:
            values['l10n_do_ncf_expiration_date']=values['ncf_expiration_date']
            del values['ncf_expiration_date']
        if 'is_l10n_do_internal_sequence' in values:
            del values['is_l10n_do_internal_sequence']
        return super(AccountMove, self).new(values, origin, ref)

    def _get_sequence_format_param(self, previous):
        if 'l10n_latam_sequence_id' in previous:
            del previous['l10n_latam_sequence_id']
        if not self._context.get("is_l10n_do_seq", False):
    ...
    def _set_next_sequence(self):
        ...
        sequence_info = self.env['sequence.cf'].search(
            [('l10n_latam_document_type_id', '=', self.l10n_latam_document_type_id.id)])

        if format_values['seq']>sequence_info.limit:
            raise UserError(
                _("Secuencia fiscal alcanzo su limite favor comunicarte con Contabilidad para que la renueven")
            )
    ...
    def write(self, vals):
        ...
        if 'state' in vals:
            if vals['state'] == 'posted':
                if len(self.ids)>0:
                    move_inf=self.env['account.move'].search([('id', '=',self.ids[0])])
                    l10n_do_fiscal_number = move_inf.l10n_do_fiscal_number
                    if l10n_do_fiscal_number:
                        sequence_info=self.env['sequence.cf'].search(
                            [('l10n_latam_document_type_id', '=', move_inf.l10n_latam_document_type_id.id)])
                        self.env['sequence.cf.move'].create(
                            {
                                'name':move_inf.l10n_do_fiscal_number,
                                'l10n_latam_document_type_id':move_inf.l10n_latam_document_type_id.id,
                                'cf_sequence':sequence_info.id,
                                'account_move':move_inf.id
                            }
                        )
                        sequence = int(l10n_do_fiscal_number[3:])
                        sequence_info.write({'current_sequence':sequence})
        return rec


    def button_print_pos(self):
        print("Se imprime en el POS")